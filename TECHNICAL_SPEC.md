...
- **Ценность.** Безопасная и прослеживаемая доставка с контролем целостности; инициатива установки и обновления — на стороне клиента; единый апплайанс, не требующий предустановленного Docker на хосте.
...
| Agent (апплайанс) | Установочный пакет на сервере клиента: контейнерная среда + логика работы с Control Plane |
...
| --- | --- | --- |
| Агент-апплайанс — Go + встроенный Docker Engine | Реализует `B5` (установка/обновление на сервере клиента) | Демон — статический Go-бинарь (ноль JVM-зависимостей, мгновенный старт, кросс-компиляция). Контейнерную среду не пишем — вендорим зрелый Docker Engine/Compose |
...
```mermaid
flowchart LR
    AG["агент-апплайанс (runtime + daemon)"]
...
| агент-апплайанс | Go + встроенный Docker Engine | `B5`,`B6`,`B9` | Установка/обновление продуктов, отчёт; ретрансляция лицензионных лизов в продукт |
...
## 2.8 Агент-апплайанс (Go + встроенный Docker) — реализация `B5`, `B6`, `B9`
Самодостаточный пакет на сервере клиента, объединяющий контейнерную среду и логику работы с Control Plane. Организация ставит **один** апплайанс — предустановленный Docker не требуется.

- **Состав поставки (installer):** встроенный Docker Engine (`dockerd` + `containerd`) и плагин Docker Compose v2 пиннутых версий; Go-демон; локальный CLI и web-UI; systemd-юниты (жизненный цикл runtime/демона, запуск enrollment при первом старте).
- **Предусловия на хосте:** поддерживаемый Linux (x86_64/arm64), systemd, root для установки. **Docker предустанавливать не требуется.**
- **Компоненты демона:**
  - `Enrollment` — token → CSR → получение mTLS-идентичности и robot-кредов Harbor (`S1`).
  - `RuntimeManager` — установка/контроль/обновление встроенного Docker Engine.
  ...
  - `SelfUpdater` — обновление самого апплайанса (daemon + runtime) по политике.
...
flowchart TB
    subgraph CLIENT["Сервер клиента (on-prem)"]
        AGT["агент-апплайанс (Go + встроенный Docker)"]
...
| Совместимость хоста | Linux x86_64/arm64, systemd (Docker предустанавливать не нужно) |
...
| Q4 | Встроенный runtime: вендоренный Docker vs containerd напрямую | Вендоренный Docker Engine + Compose v2 |
...
- Подписанные пакеты установки и self-update апплайанса (§2.12) подтверждают происхождение распространяемых артефактов.
...
| Компрометация апплайанса | Подписанные пакеты установки и self-update, пиннутые версии runtime | §2.12, §3.7 |
